from imitation.common.misc_util import boolean_flag


class DotDict(dict):
    """Create a dictionary-like structure that inherits from the python dict class and
    allows for the dot notation, e.g. mydict.att1 as opposed to the usual mydict[att1]
    The notation makes the structure virtually behave like argparse.Namespace,
    while having the slew of methods available with dictionaries.
    """
...
def disambiguate(kvs, tokens, verbose=False):
    """For each token, return a `DotDict` whose keys do not start
    with any other token from the list of tokens, and wrap in a tuple
    """
    # List out ambiguous hyperparameter stems
    amb_hp_stems = ['nums_filters', 'filter_shapes', 'stride_shapes',
                    'hid_widths', 'ent_reg_scale']
    import argparse
    ...
    for token in tokens:
        ...
        for k, v in kvs.items():
            ...
            if verbose:
                print("key: {:15} | head: {:15} | queue: {:15}".format(k, k_h, k_q))
        ...
        subdict = DotDict(subdict)
        if verbose:
            print("refined dict: {}".format(subdict))
        listofdicts.append(subdict)
    return tuple(listofdicts)
