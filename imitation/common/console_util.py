...
from imitation.common.misc_util import zipsame, prettify_time
...
def timed_cm_wrapper(comm=None, logger=None, color_message='magenta', color_elapsed_time='cyan'):
    ...
    def _timed(message):
        ...
        """
        display = print if logger is None else logger.info
        if comm is None or comm.Get_rank() == 0:
            display(colorize(message, color=color_message))
            tstart = time.time()
            yield
            display(colorize("  [done in {:.3f} seconds]".format(time.time() - tstart),
